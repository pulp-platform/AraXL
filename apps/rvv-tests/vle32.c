...
uint32_t res[20][256] __attribute__((aligned(AXI_DWIDTH)));
void TEST_CASE20(void) {
  ...
  for (int i=0; i<20; i++) {
    ...
    asm volatile("vsetvli %0, %1, e32, m8, ta, ma" : "=r"(avl) : "r"(vl));
    asm volatile("vle32.v v8, (%0)"::"r"(&LONG_I32[misalign]));
    asm volatile("vse32.v v8, (%0)"::"r"(res[i]));

    for (int idx=0; idx<vl; idx++) {
      if (res[i][idx]!=LONG_I32[idx+misalign]) {
        printf("Index error at idx:%d expected:%d got:%d\n",idx,LONG_I32[idx+misalign],res[i][idx]);
        return;
      }
    }
