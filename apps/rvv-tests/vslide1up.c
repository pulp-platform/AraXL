...
void TEST_CASE3(void) {
  printf("Running vl regression tests for slide1up SEW=8\n");
  int avl, vl;
...
void TEST_CASE4(void) {
  printf("Running vl regression tests for slide1up SEW=16\n");
  int avl, vl;
...
void TEST_CASE5(void) {
  printf("Running vl regression tests for slide1up SEW=32\n");
  int avl, vl;
...
// Test cases to load irregular vector lengths and misaligned start address
uint64_t res_64[20][256] __attribute__((aligned(4 * NR_LANES * NR_CLUSTERS), section(".l2")));
void TEST_CASE6(void) {
  printf("Running vl regression tests for slide1up SEW=64\n");
  int avl, vl;
  ...
  for (int i=0; i<20; i++) {
    ...
    asm volatile ("vle64.v v2, (%0)"::"r"(LONG_I64));
    asm volatile ("vslide1up.vx v8, v2, %0"::"r"(a));
    asm volatile ("vse64.v v8, (%0)"::"r"(res_64[i]));
    ...
    for (int idx = 0; idx < vl; idx++) {
      if (idx == 0) {
        ...
        if (res_64[i][idx] != LONG_I64[idx-1]) {
          printf("Error at index %d: expected %lx, got %lx\n", idx, LONG_I64[idx-1], res_64[i][idx]);
          num_failed++;
          return;
        }
      }
