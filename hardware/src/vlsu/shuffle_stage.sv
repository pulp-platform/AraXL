...
always_comb begin
  ...
  if (axi_req_i[cluster_ar_q].ar_valid & axi_resp_o[cluster_ar_q].ar_ready) begin
    ...
    if ((cluster_metadata.op == VLXE) || (cluster_metadata.op == VLSE)) begin
      lane_ar_d += 1;
  ...
  pending_resp = ((rd_datapath == SHUFFLE) || (rd_op == VLXE) || (rd_op == VLSE)) && (|buf_valid_q);
  ...
  else if ((rd_op == VLXE || rd_op == VLSE) & ~pending_resp) begin
...
assign r_valid[0]   = (rd_datapath == BUFFER) ? 1'b0 : (rd_op == VLXE || rd_op == VLSE) ? 1'b0 : axi_resp_i[0].r_valid;
...
for (genvar c=0; c < NrClusters; c++) begin
  ...
  assign axi_req_o[c].ar_valid = (((cluster_metadata_i[c].op == VLXE) || (cluster_metadata_i[c].op == VLSE)) ? ((c==cluster_ar_q) ? axi_req_i[c].ar_valid : 1'b0) : axi_req_i[c].ar_valid) && !rd_full;
  assign axi_req_o[c].b_ready = axi_req_i[c].b_ready;
  
  // Reads
  assign axi_req_o[c].r_ready = ((rd_datapath == BUFFER) ? (((rd_op == VLXE) || (rd_op == VLSE)) ? axi_req_i[c].r_ready : r_ready_buf_q) : r_ready[0] ) & ~pending_resp;
