...
always_comb begin : p_global_ldst
  ...
  if (axi_req_i[cluster_ar_q].ar_valid && r_req_ready) begin
    ...
    // If not an indexed load, always use cluster 0
    if ((cluster_metadata_i.op != VLXE) && (cluster_metadata_i.op != VLSE)) begin
      cluster_ar_d = '0;
    end else begin
      // Increment request counter for current cluster
  ...
  if (r_req_valid_d==1'b1 && axi_resp_i.ar_ready) begin
    ...
    automatic vlen_cluster_t vl_req = ((cluster_metadata_i.op == VLXE) || (cluster_metadata_i.op == VLSE)) ? 1 : vl_ldst_rd_d;
  ...
  if (!fifo_empty_o) begin
    ...
    if (axi_resp_i.r_valid) begin : p_valid_read_resp
      ...
      for (int i=0; i<(AxiDataWidth/ClusterAxiDataWidth); i++) begin
        if ((idx_metadata_o.op == VLXE) || (idx_metadata_o.op == VLSE)) begin
    ...
    for (int i=0; i<NrClusters; i++) begin  
      ...
      if ((idx_metadata_o.op == VLXE) || (idx_metadata_o.op == VLSE)) begin
        // // For VLSE, the axi req is always issued from the cluster 0, but the response should be sent to the desired cluster
        // if (idx_metadata_o.op == VLSE) begin
        //   axi_resp_o[i].r = (i == cluster_r_q) ? cluster_axi_resp_data_d[0].r : '0;
        //   axi_resp_o[i].r_valid = (i == cluster_r_q) ? cluster_axi_resp_data_d[0].r_valid : 1'b0;
        // end else begin
    ...
    if (((idx_metadata_o.op != VLXE) && (idx_metadata_o.op != VLSE)) && axi_resp_o[0].r_valid && axi_req_i[0].r_ready) begin
